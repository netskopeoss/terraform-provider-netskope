...
// TestAccDeviceClassificationRule_withDeviceTagCheck verifies that a classification
// rule can reference a Device Tag via the device_tag_check condition — the integration
// pattern described in the "Device Tags for Classification" guide. Requires the Device
// Tag API to be enabled on the tenant (gate: NETSKOPE_TEST_DEVICE_TAGS=1).
func TestAccDeviceClassificationRule_withDeviceTagCheck(t *testing.T) {
	rName := fmt.Sprintf("%s-%s", testutil.ResourcePrefix, acctest.RandString(8))
	resourceName := "netskope_device_classification_rule.test"
	vars := config.Variables{
		"name": config.StringVariable(rName),
