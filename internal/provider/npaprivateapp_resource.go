...
func (r *NPAPrivateAppResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	silent := new(operations.Silent)
	if !data.Silent.IsUnknown() && !data.Silent.IsNull() {
		*silent = operations.Silent(data.Silent.ValueString())
	} else {
		silent = nil
	}
	privateAppsRequest := *data.ToSharedPrivateAppsRequest()
	request := operations.CreateNPAPrivateAppsRequest{
		Silent:             silent,
		PrivateAppsRequest: privateAppsRequest,
	}
	res, err := r.client.CreateNPAPrivateApps(ctx, request)
	if err != nil {
...
func (r *NPAPrivateAppResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	res, err := r.client.UpdateNPAPrivateApp(ctx, request)
	if err != nil {
		resp.Diagnostics.AddError("failure to invoke API", err.Error())
		if res != nil && res.RawResponse != nil {
			resp.Diagnostics.AddError("unexpected http request/response", debugResponse(res.RawResponse))
