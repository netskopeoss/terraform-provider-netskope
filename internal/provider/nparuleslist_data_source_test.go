...
func TestAccNPARulesListDataSource_basic(t *testing.T) {
	rName := fmt.Sprintf("%s-%s", testAccResourcePrefix, acctest.RandString(8))
	dataSourceName := "data.netskope_npa_rules_list.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNPARulesListDataSourceConfig_basic(rName),
...
func testAccNPARulesListDataSourceConfig_basic(name string) string {
	return fmt.Sprintf(`
%s
...
resource "netskope_npa_rules" "test" {
  rule_name   = %q
  description = "List data source test rule"
  enabled     = "1"
...
data "netskope_npa_rules_list" "test" {
  depends_on = [netskope_npa_rules.test]
}
