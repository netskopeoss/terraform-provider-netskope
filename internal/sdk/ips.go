...
func newIps(rootSDK *TerraformProviderNs, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Ips {
	return &Ips{
		rootSDK:          rootSDK,
...
// GetIPSStatus - Get IPS enablement status
// Retrieve the current IPS (Intrusion Prevention System) enablement status
// for web, non-web, and NPA traffic types.
func (s *Ips) GetIPSStatus(ctx context.Context, opts ...operations.Option) (*operations.GetIPSStatusResponse, error) {
	...
	opURL, err := url.JoinPath(baseURL, "/ips/status")
	if err != nil {
	...
	res := &operations.GetIPSStatusResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out shared.IpsStatusResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.IpsStatusResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
