...
type AIGRateLimits struct {
	rootSDK          *TerraformProviderNs
	sdkConfiguration config.SDKConfiguration
	hooks            *hooks.Hooks
}

func newAIGRateLimits(rootSDK *TerraformProviderNs, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *AIGRateLimits {
	return &AIGRateLimits{
		rootSDK:          rootSDK,
...
// ListObjects - List Rate Limit Rules
// Returns a paginated list of AI Gateway rate limit rules.
func (s *AIGRateLimits) ListObjects(ctx context.Context, opts ...operations.Option) (*operations.ListAigRateLimitsResponse, error) {
	...
	res := &operations.ListAigRateLimitsResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out shared.AigRateLimitListResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.AigRateLimitListResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
