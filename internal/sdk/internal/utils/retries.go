...
//
// This file is listed in .genignore and will NOT be overwritten by speakeasy run.
// Two bugs in the generated retry logic have been fixed manually:
//   1. Retry-After header values are now capped to MaxInterval so a large
//      server-supplied value cannot stall the provider for tens of minutes (#108).
//   2. The elapsed-budget check now runs before sleeping, not after, so the
//      provider fails fast when time has run out rather than sleeping a full
//      Retry-After window then failing (#108).
// See internal/sdk/internal/utils/retries_test.go for regression tests.
