...
type AIGAppliances struct {
	rootSDK          *TerraformProviderNs
	sdkConfiguration config.SDKConfiguration
	hooks            *hooks.Hooks
}

func newAIGAppliances(rootSDK *TerraformProviderNs, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *AIGAppliances {
	return &AIGAppliances{
		rootSDK:          rootSDK,
...
// ListObjects - List AIG Appliances
// Returns a paginated, sortable, filterable list of AI Gateway appliances.
func (s *AIGAppliances) ListObjects(ctx context.Context, opts ...operations.Option) (*operations.ListAigAppliancesResponse, error) {
	...
	res := &operations.ListAigAppliancesResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out shared.AigApplianceListResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.AigApplianceListResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
