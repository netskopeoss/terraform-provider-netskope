...
resource "netskope_npa_private_app" "multi_host" {
  ...
  private_app_hostname = "10.33.5.143,webapp02.example.com"
...
- Config sends: `"10.33.5.143,webapp02.example.com"` (no spaces)
- API returns: `"10.33.5.143, webapp02.example.com"` (space after comma)
...
```
Config: "10.33.5.143,webapp02.example.com"
  → Create/Update request: host = "10.33.5.143,webapp02.example.com"
  → API response: host = "10.33.5.143, webapp02.example.com"  (space added)
  → State: "10.33.5.143, webapp02.example.com"
...
Reported by customer running the `infrastructure-netskope-private-access` module with YAML-driven app definitions. Multiple YAML files use comma-separated hostnames with spaces (e.g. `"10.33.5.143, webapp02.example.com"`). Every `terraform plan` shows in-place updates for these resources even when no configuration has changed.
