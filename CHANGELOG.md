...
- **`netskope_npa_rules` `periodic_reauth` template lost on every update; display names not stored in state** ([#118](https://github.com/netskopeoss/terraform-provider-netskope/issues/118)) — Two related bugs: (1) The BeforeRequest hook was stripping `.html` template filenames from **all** update payloads regardless of `action_name`. For `block` rules this is correct (the API preserves the existing template when the field is omitted); for `periodic_reauth` rules it caused the API to return an error on every update ("template field is required"). (2) The AfterSuccess hook failed to translate the API-returned `.html` filename back to a display name for `periodic_reauth` rules, so state stored the filename, causing drift and import failures. Fixed by: scoping the `.html` strip to `block` rules only; adding a session-level cache populated from `/api/v2/templates/usernotifications` that translates filenames back to display names for all action types (with a global fallback for cross-action-type templates); and correcting the OAS `notify.templates` type (was `array of array of string`, is `array of object`). Users configure `template = "My Reauth Template"` — the API accepts display names natively and translates them server-side. `terraform import` now correctly stores the display name in state. For tenants where the templates API is unavailable, supply the filename directly (e.g. `"10.html"`). See [KNOWN_API_ISSUES #13](docs/KNOWN_API_ISSUES.md#13-template-field--different-behavior-for-block-vs-periodic-reauth-rules).
- **`netskope_npa_rules` `notify` removed from schema; import and all plan/apply operations panicked for rules with non-standard `notify.templates` shape** — `notify` was added to the schema in v0.4.9 but is API-computed and not user-configurable. Rules using custom AD attribute notifications returned `notify.templates` as an array of objects rather than the expected array of arrays, causing `cannot unmarshal object into []json.RawMessage` on every import or state refresh of such rules. A separate bug caused `no attribute notify` panics on all plan and apply operations after the field was internally removed. Fixed by removing `notify` from the Terraform schema entirely and correcting the `Notify.Templates` type in the hook model (`[][]string` → `[]map[string]any`). **Breaking change:** Remove any `notify` blocks from HCL configurations — they will fail schema validation on upgrade.
- **`netskope_service_object` omitted protocol lists were sent to the API as empty arrays, causing the UI to record them as "Any" port** ([#119](https://github.com/netskopeoss/terraform-provider-netskope/issues/119)) — When `udp` or `tcp_udp` were omitted from the `protocols` block, the SDK generated non-nil empty slices (`[]string{}`). Speakeasy's JSON marshaler serializes `[]string{}` as `[]` (its `omitempty` only omits nil, not empty slices). The API interprets an empty port array as "Any port" — semantically different from the key being absent — so omitting a protocol inadvertently opened all ports for that protocol. Fixed by a BeforeRequest hook on `createServiceObject` and `updateServiceObject` that strips protocol keys whose value is an empty array before the request reaches the API.
...
- **`netskope_rbac_role_config`** data source — Read-only catalog of RBAC role configuration from `GET /api/v2/rbac/roles/config`. Exposes `api_groups` (tenant-specific IDs, names, display names, max permission levels), `obfuscation` (field catalog keyed by api_group_id), and `functions` (UI groupings mapping function names to API group IDs). Resolves the portability problem where `netskope_rbac_role` resources required hardcoded tenant-specific API group IDs. ([#109](https://github.com/netskopeoss/terraform-provider-netskope/issues/109))
- **New `netskope_npa_rules` schema fields** — `rule_data` now exposes `schedule` (time-window restrictions), `periodic_reauth` (re-authentication interval), `notify` (user notification settings), `user_confidence` (user confidence level filter), `private_app_tag_ids` (read-only tag ID list), and `os` (operating system filter — `"Windows"`, `"macOS"`, `"Linux"`, `"Android"`, `"iOS"`, `"Chrome OS"`; Client access only). These fields were absent from the schema and were previously silently dropped on every read and write. ([#111](https://github.com/netskopeoss/terraform-provider-netskope/issues/111))
- **Retry configuration provider attributes** — New optional provider attributes `retry_max_elapsed_time` (seconds) and `retry_disabled` (bool) allow per-workspace retry tuning without environment variables. Both fall back to environment variables (`NETSKOPE_RETRY_MAX_ELAPSED`, `NETSKOPE_RETRY_DISABLED`) and then to the built-in defaults (300 s / false). CI pipelines can set `retry_max_elapsed_time = 30` to fail fast on rate limits.
- **Provider-level OAuth2 authentication** — The provider now accepts OAuth2 client credentials as an alternative to API key authentication. Set `NETSKOPE_OAUTH2_CLIENT_ID` and `NETSKOPE_OAUTH2_CLIENT_SECRET` to authenticate all API calls via bearer token without needing `NETSKOPE_API_KEY`. Tokens are fetched transparently per-request, cached for their lifetime, and refreshed automatically on 401. API key takes precedence: if `NETSKOPE_API_KEY` is also set, OAuth2 is not used. The OAuth2 client must be configured in the tenant with sufficient API permissions for the resources being managed.

### Fixed
- **Rate-limited operations could hang for up to 29 minutes** ([#108](https://github.com/netskopeoss/terraform-provider-netskope/issues/108)) — Two bugs in the retry backoff logic: (1) a `Retry-After` header value was applied without capping to `MaxInterval`, allowing a server-supplied value (e.g. 1740 s) to override the configured ceiling; (2) the elapsed-budget check ran after sleeping, so the provider slept the full `Retry-After` window even when it exceeded the remaining budget. Fixed by capping `Retry-After` to `MaxInterval` and checking the budget before sleeping. A `tflog.Warn` line now logs the wait duration and attempt number so operators can distinguish a rate-limited wait from a hang.
- **`netskope_npa_rules` AfterSuccess and BeforeRequest hook structs were missing fields** — The hook structs (`models.RuleData`, BeforeRequest model) were missing `notify`, `schedule`, `periodic_reauth`, `user_confidence`, `user_group_objects`. Any rule using these fields would lose them on the next state refresh or update.
- **`netskope_npa_rules` read crashes when `rule_data.classification` is set** ([#106](https://github.com/netskopeoss/terraform-provider-netskope/issues/106)) — The AfterSuccess hook deserialized `classification` into a `*string` field, but the API returns it as `[]string` for rules with a managed/unmanaged device classification. Any `terraform import` or state refresh of such a rule crashed with `json: cannot unmarshal array into Go struct field RuleData.data.rule_data.classification of type string`. Fixed by correcting the field type in the hook model structs.
- **`netskope_npa_rules` block rules produce a phantom update on every plan, and any apply fails with "Undefined template: *.html"** ([#105](https://github.com/netskopeoss/terraform-provider-netskope/issues/105)) — Two root causes: (1) `group_name` is Computed-only but never returned by GET-by-ID, leaving it `(known after apply)` and keeping the plan dirty after template drift was suppressed; (2) `suppressTemplateDrift` set the planned template to the state file name (e.g. `"23.html"`), which leaked into PUT bodies and caused the API to reject every update. Fixed by extending `preserveComputedStringAttr` to handle null state (eliminating the `group_name` phantom) and stripping `.html` file names from update payloads in the BeforeRequest hook. Block rules can now be fully managed via Terraform.
