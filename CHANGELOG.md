...
- **Provider-level OAuth2 authentication** — The provider now accepts OAuth2 client credentials as an alternative to API key authentication. Set `NETSKOPE_OAUTH2_CLIENT_ID` and `NETSKOPE_OAUTH2_CLIENT_SECRET` to authenticate all API calls via bearer token without needing `NETSKOPE_API_KEY`. Tokens are fetched transparently per-request, cached for their lifetime, and refreshed automatically on 401. API key and OAuth2 are mutually exclusive; if both are set, the OAuth2 bearer token takes precedence.
